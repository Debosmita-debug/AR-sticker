...
export default function ARScannerClient() {
  ...
  const initMindAR = useCallback(async (pwd?: string) => {
    ...
    try {
      ...
      // Create video element with CORS support
      const video = document.createElement('video');
      video.loop = data.options.loop !== false;
      video.muted = true;
      video.playsInline = true;
      video.preload = 'metadata'; // Preload metadata to check if video loads
      video.crossOrigin = 'anonymous'; // CORS support for Cloudinary
      ...
      video.addEventListener('error', (e) => {
        ...
        setErrorMsg(`Video failed to load: ${video.error?.message || 'Unknown error'}`);
      });
      ...
      anchor.onTargetFound = () => {
        ...
        console.log('[AR] Target detected! Loading video:', data.videoUrl);
        ...
        if (!texture) {
          try {
            ...
            video.load();
            console.log('[Video] Load() called for:', data.videoUrl);
            ...
            if (playPromise !== undefined) {
              playPromise
                ...
                .catch((error: any) => {
                  ...
                  setErrorMsg(`Video play failed: ${error.message}`);
                });
            }
          } catch (err: any) {
            console.error('[Video] Setup error:', err);
            setErrorMsg(`Video setup failed: ${err.message}`);
          }
        } else {
